...
public class UserService {
    ...
    public void codeExist(String code){
        try{
            DatabaseDao databaseDao=new DatabaseDao();
            UserDao userDao = new UserDao();
            userDao.hasCode(code,databaseDao);
        }catch (SQLException e){
            e.printStackTrace();
        } catch (Exception e) {
            e.printStackTrace();
        ...
        try {
            ...
            while (iter.hasNext()) {//遍历表单元素
                ...
                if (item.isFormField()) {//非上传文件表单元素
                    ...
                    if("signature".equals(item.getFieldName()))//获取表单元素的name属性
                        userinformation.setSignature(item.getString("UTF-8"));//获取表单元素的值（一般是value属性）
                } else if(!item.isFormField()){//上传文件
                    File uploadedFile = null;
                    String randomFileName;
                    do{
                        randomFileName=FileTool.getRandomFileNameByCurrentTime(item.getName());
                        String full=request.getServletContext().getRealPath(WebProperties.config.getString("headIconDirDefault")+"\\"+randomFileName);
                        if(full==null)
                            break;
